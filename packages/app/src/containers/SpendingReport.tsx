//
import { Flex, Grid, GridProps, Spinner, Tab, TabList, TabPanel, TabPanels, Tabs, Text, useMediaQuery } from '@chakra-ui/react';
...
const SpendingReport: React.FC<SpendingReportProps> = ({ wallet, ...props }) => {
	...
	const [spendingDate, setSpendingDate] = React.useState<string>(queryParams.has('date') ? queryParams.get('date') ?? moment().format('MM-YYYY') : moment().format('MM-YYYY'));
	...
	const spendingReportQuery = useQuery<Record<string, any>>({
		...
		queryFn: async () => {
			try {
				...
				const { status, data: { data } } = await api.getSpendingReport({
					params: {
	...
	const updateSpendingDate = (action: 'prev' | 'next') => {
		...
		setSpendingDate((prevDate) => {
			const nextDate = moment(prevDate, 'MM-YYYY')[action === 'prev' ? 'subtract' : 'add'](1, 'month');
		...
		>
			<Flex flexDirection="column" alignItems="center" justifyContent="center" p="3.6rem" gap="2rem" maxW="calc(100vw - (2 * var(--tuturium-space-mobile-spacing-content-x)) - 0.5rem)">
				<Flex gap="1rem" alignItems="center">
					...
					<Text as="span" bg="purple.800" color="purple.200" fontSize="2rem" fontWeight={500} lineHeight={1} textAlign="center" px="3.2rem" py="1.25rem" borderRadius="lg">{moment(spendingDate, 'MM-YYYY').format('MMM YYYY')}</Text>
					<Button variant="circle-icon" icon="general.chevron-right" onClick={() => updateSpendingDate('next')} />
				</Flex>

				<Flex gap="1rem" alignItems="center">
					<Button variant="circle-icon" icon="general.chevron-left" onClick={() => updateSpendingWeek('prev')} isDisabled={isNull(spendingWeek) || spendingWeek === 0} />
					...
					<Button variant="circle-icon" icon="general.chevron-right" onClick={() => updateSpendingWeek('next')} isDisabled={spendingWeek === 4} />
				</Flex>
